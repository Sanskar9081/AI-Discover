...
import { useCategories, useTools, usePrompts, useAds, useUsers, useSettings, useUseCases, useNewsletterSubscriptions, useBugReports, useAdRequests, useContactMessages } from "@/hooks/useQueries";
import { useCreateTool, useUpdateTool, useDeleteTool, useCreatePrompt, useUpdatePrompt, useDeletePrompt, useCreateAd, useUpdateAd, useDeleteAd, useApproveToolMutation, useRejectToolMutation, useApprovePromptMutation, useRejectPromptMutation, useUpdateSettings, useUpdateToolCoupon, useCreateUseCase, useUpdateUseCase, useDeleteUseCase, useUpdateNewsletterFrequency, useUnsubscribeNewsletter, useDeleteNewsletterSubscription, useDeleteBugReport, useDeleteAdRequest, useUpdateBugStatus, useDeleteContactMessage, useUpdateContactMessageStatus } from "@/api/mutations";
...
const Admin = () => {
  ...
  const { data: adRequests = [] } = useAdRequests();
  const { data: contactMessagesList = [] } = useContactMessages();
                ...
                <tbody>
                  {bugReports.length === 0 ? (
                    ...
                    bugReports.map((bug: any) => (
                      <tr key={bug._id} className="border-b border-border/40 hover:bg-secondary/50">
                        <td className="px-6 py-4">
                          <div className="font-medium">{bug.title}</div>
                          <div className="text-xs text-muted-foreground truncate max-w-[200px]">{bug.description}</div>
                        </td>
                        <td className="px-6 py-4">{bug.page_url}</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            bug.severity === 'high' ? 'bg-destructive/20 text-destructive' :
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                            bug.status === 'resolved' ? 'bg-green-500/20 text-green-500' :
                            bug.status === 'in_progress' ? 'bg-blue-500/20 text-blue-500' :
                            'bg-muted text-muted-foreground'
                          }`}>
                            {bug.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-sm">{bug.contact_email || 'N/A'}</td>
                        <td className="px-6 py-4">
                          <div className="flex gap-2">
                            {/* Action buttons... */}
                          </div>
                ...
                <tbody>
                  {adRequests.length === 0 ? (
                    ...
                    adRequests.map((req: any) => (
                      <tr key={req._id} className="border-b border-border/40 hover:bg-secondary/50">
                        <td className="px-6 py-4">
                          <div className="font-medium">{req.business_name}</div>
                          <div className="text-xs text-muted-foreground truncate max-w-[200px]">{req.product_description}</div>
                        </td>
                        <td className="px-6 py-4 text-sm">
                          <a href={req.website_url} target="_blank" rel="noreferrer" className="text-accent hover:underline">
                            {req.website_url}
                          </a>
                        </td>
                        <td className="px-6 py-4 font-medium">{req.budget}</td>
                        <td className="px-6 py-4 text-sm">{req.contact_email}</td>
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${
